...
conn = sqlite3.connect("./Practical-5/avto.db", check_same_thread=False)
cursor = conn.cursor()
...
def check_id(ad_name, user_id):
    cursor, conn = get_cursor()  # Отримуємо новий курсор та з'єднання
    cursor.execute('SELECT id FROM avto WHERE user_id=? AND brand=?', (user_id, ad_name))
    data = cursor.fetchone()
    cursor.close()  # Закриваємо курсор після використання
...
def find(user_id, brand):
    ...
    for row in rows:
        ...
        if photos:
            ...
            photos[0].parse_mode = 'HTML'  # Додаємо parse_mode для першого фото з текстом
            bot.send_media_group(user_id, media=photos)
            # Закриваємо файли перед видаленням каталогу
            for photo in photos:
                photo.media.close()
        else:
            bot.send_message(user_id, text=text_message, parse_mode='HTML')

        # Видаляємо каталог після закриття файлів
